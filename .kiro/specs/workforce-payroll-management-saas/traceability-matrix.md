...
|---|---|---|---|---|
| [R1 Tenant Isolation and Authorization Boundary](requirements.md#requirement-1-tenant-isolation-and-authorization-boundary), AC 1.1–1.10 | §§4.4, 5.3, 7.3, 20.1–20.2; Property 1 | 2.1–2.4, 3.2 | 3.5, 25.7, 29.1 (P1/P21), 30.5 | `VERIFIED` for R1.1–R1.4 and R1.6, and for R1.5 as applied to Task 2.1 Principal persistence, by evidence `TR-005`–`TR-007`; R1.5's later storage/channel contracts and R1.7–R1.10 remain `SPECIFIED` for Tasks 2.2–3.5. |
...
| [R43 Correctness and Testability](requirements.md#requirement-43-correctness-and-testability), AC 43.1–43.14 | §§23–24 | Test-bearing subtasks throughout 3–28; 29.1–29.9 | 29.1–29.9, 30.1–30.10 | `VERIFIED` for the Task 2.1-applicable unit, integration, security, edge, and Property 1 obligations by `TR-005`–`TR-007`; remaining feature-wide quality obligations stay `SPECIFIED`. |
...
| `TR-005` | R1.1, R1.5–R1.6 | 2.1 | Task 2.1 unit and HTTP integration verification | `internal/identity.JWTVerifier`, `identity.Authenticator`, `identity.PostgresPrincipalRepository.ResolveActive`, `tenancy.WithPrincipal`, `tenancy.EffectiveTenant`, `httpapi.Authenticate`, `httpapi.EnforceSuppliedTenant`, routes `GET /v1/context` and `GET /v1/tenants/{tenantID}/context`, and migration `000001_principal_bindings` | `internal/identity/jwt_verifier_test.go` `TestJWTVerifierAcceptsHumanAndIntegrationPrincipals`, negative token tests; `internal/integration/task21_integration_test.go` `TestTask21PrincipalAndTenantBoundaryIntegration`; unit, edge, integration, security | `PASS`; `make check`: `go vet ./...`, race-enabled complete suite, real PostgreSQL 17.6 integration, and both binaries built; Task 2.1 commit containing this record | Section 3.1 Task 2.1 stack approval only; no Tenant or production identity configuration activated. `G-TENANT-ROLLOUT`, `G-PAYROLL-PRODUCTION`, `G-PRODUCTION-OPERATIONS`, and `G-RELEASE` remain closed. | none |
| `TR-006` | R1.2–R1.4 | 2.1 | Task 2.1 Tenant-boundary integration/security verification | `internal/tenancy.CompareSuppliedTenant`, `tenancy.LoadResource`, `tenancy.ExecuteMutation`, Tenant-scoped repository contract `ScopedResourceReader.FindInTenant`, uniform API `RESOURCE_NOT_FOUND`, and server-derived path/header/query checks | `internal/tenancy/guard_test.go` `TestCompareSuppliedTenant`, `TestLoadResourceDoesNotDiscloseCrossTenantExistence`; integration subtests `cross-Tenant path and header substitutions have identical non-disclosing denial` and `disabled and unknown Principals share safe unauthenticated response`; unit, integration, security | `PASS`; `make check` race-enabled suite with complete persisted-row snapshot assertion proving denial leaves authoritative Principal contents unchanged; Task 2.1 commit containing this record | No policy value introduced; cross-Tenant and malformed supplied Tenant values converge on a safe outcome. No rollout or release gate opened. | none |
| `TR-007` | R1.1–R1.4, R1.6; R43.2–R43.5 as applicable | 2.1 | Task 2.1 Property 1 verification | `internal/tenancy.ExecuteMutation`, `tenancy.LoadResource`, `tenancy.CompareSuppliedTenant`, and immutable server-derived `foundation.Identifier` Tenant context | `internal/tenancy/guard_test.go` `TestProperty1TenantIsolation`; property; **Feature: workforce-payroll-management-saas, Property 1: Tenant Isolation**; **Validates: Requirements 1.1, 1.2, 1.3, 1.4, 1.6** | `PASS`; Rapid `v1.3.0`, 200 generated actor/resource/command checks across human and Integration Principal kinds under the race detector; guarded mutation invocation, effective-Tenant repository argument, no-call-on-substitution, shrinking, and deterministic replay verified; Task 2.1 commit containing this record | No generated Tenant becomes configured authority; only the server-derived Tenant is effective. PBT-status metadata was not changed because Task 2.1 is not classified as a PBT task by the task system. | none |
...
- Task 2.1 application implementation exists and is evidenced by `TR-005`–`TR-007`; later tasks retain their prior specified or gated states.
...
- `G-IMPLEMENTATION` is partially open only for the exact Task 2.1 choices in the decision register; `G-TENANT-ROLLOUT`, `G-PAYROLL-PRODUCTION`, `G-PRODUCTION-OPERATIONS`, and `G-RELEASE` remain closed.
